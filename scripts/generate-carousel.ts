...
async function generateSlide1(
  ...
  const heroUrl =
    "https://res.cloudinary.com/duiyn8wll/image/upload/_Z9_20260212_100758_TMH_website_s3qioq.jpg";

  process.stdout.write("  Fetching photo... ");
  const imgBuffer = await fetch(heroUrl)
        ...
        // Bottom gradient — photo breathes at top, fades to black at bottom
        {
          ...
          props: {
            style: {
              ...
              backgroundImage:
                "linear-gradient(to bottom, rgba(8,8,8,0), rgba(8,8,8,0.75) 70%, rgba(8,8,8,0.9))",
            },
          },
        },
...
async function generateSlide2(
  ...
  process.stdout.write("  Fetching photo... ");
  const imgBuffer = await fetch(photoUrl)
    .then((r) => r.arrayBuffer())
    .then((a) => Buffer.from(a));
  console.log("done");
        ...
        // Bottom gradient — photo breathes at top, fades to dark at bottom
        {
          ...
          props: {
            style: {
              ...
              backgroundImage:
                "linear-gradient(to bottom, rgba(8,8,8,0), rgba(8,8,8,0.75) 55%, rgba(8,8,8,0.9))",
            },
          },
        },
        // Text container — flex column anchored to bottom
        ...
        // Dark panel at bottom
        {
          ...
          props: {
            style: {
              ...
              backgroundImage:
                "linear-gradient(to top, rgba(8,8,8,0.7), rgba(8,8,8,0))",
            },
          },
        },
        // Text container — flex column anchored to bottom
            ...
            children: [
              ...
              },
              {
                ...
                props: {
                  ...
                  children:
                    "Soft winter light. Sambar crossing rivers in no rush. Tigers patrolling the Ramganga.",
                },
              },
            ],
          },
...
async function generateSlide4(
  ...
  const body =
    "Before the first barking deer. Before the first jeep. Just this.";
            ...
            children: [
              ...
              },
              {
                ...
                props: {
                  ...
                  children:
                    "Camera ready. Settings dialed in. The forest does the rest.",
                },
              },
            ],
          },
...
async function generateSlide6(
  ...
  const [leftBuf, topRightBuf, botRightBuf] = await Promise.all([
    ...
    fetch(images[2].image.url)
      .then((r) => r.arrayBuffer())
      .then((a) => Buffer.from(a)),
    // Top-right: Sal canopy light
    fetch(images[0].image.url)
      .then((r) => r.arrayBuffer())
      .then((a) => Buffer.from(a)),
    // Bottom-right: Sal road
    fetch(images[1].image.url)
      .then((r) => r.arrayBuffer())
      .then((a) => Buffer.from(a)),
  ]);
  console.log("done");
  ...
  const [leftImg, topRightImg, botRightImg] = await Promise.all([
    sharp(leftBuf)
      .resize(LEFT_W, GRID_H, { fit: "cover", position: "centre" })
      .png()
      .toBuffer(),
    sharp(topRightBuf)
      .resize(RIGHT_W, RIGHT_H, { fit: "cover", position: "centre" })
      .png()
      .toBuffer(),
    sharp(botRightBuf)
      .resize(RIGHT_W, RIGHT_H, { fit: "cover", position: "centre" })
...
async function main() {
  ...
  for (const { label, fn } of slides) {
    ...
    console.log(
      `  ✓ .claude/skills/social-carousel/output/${today}/${filename}`,
    );
  }
}
