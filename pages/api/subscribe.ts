...
export default async function handler(
  ...
  if (!apiSecret || !formId) {
    console.error("Missing KIT_API_SECRET or KIT_FORM_ID environment variables");
  ...
  try {
    ...
    if (!createRes.ok) {
      ...
      const status = createRes.status >= 400 && createRes.status < 500 ? 400 : 500;
      const message = status === 400 ? "Please enter a valid email address" : "Failed to subscribe";
      return res.status(status).json({ error: message });
    }

    const createJson = (await createRes.json()) as { subscriber?: { id?: number } };
